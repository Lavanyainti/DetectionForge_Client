...
export const Route = createFileRoute("/company/$slug")({
  loader: ({ params }) => {
    const page = getSubPage("company", params.slug);
    if (!page) throw notFound();
    return { page };
  },
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.page.title ?? "Company"} — Detection Forge` },
      { name: "description", content: loaderData?.page.description ?? "" },
      { property: "og:title", content: `${loaderData?.page.title ?? "Company"} — Detection Forge` },
  ...
  notFoundComponent: () => (
    <PageShell>
      <PageHero eyebrow="Company" title="Page not found" description="The page you're looking for doesn't exist." />
    </PageShell>
  ),
  component: SubPage,
});
...
function SubPage() {
  ...
  const section = getSection("company")!;
  return (
    <PageShell>
